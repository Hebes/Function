...
public class Debugger : MonoBehaviour
{
    ...
    private void ExpansionGUIWindow(int windowId)
    {
        ...
        if (_debugType == DebugType.Console)
        {
            ...
            _scrollCurrentLogView = GUILayout.BeginScrollView(_scrollCurrentLogView, "Box", GUILayout.Height(150));
    ...
    private void HandleResize()
    {
        ...
        // 在窗口底部绘制调整大小的手柄
        Rect resizeHandle = new Rect(9, _windowRect.height - 25, _windowRect.width - 18, 25);
        GUI.Box(resizeHandle, "拖拽调整大小");

        switch (currentEvent.type)
        {
            case EventType.MouseDown:
                if (resizeHandle.Contains(currentEvent.mousePosition))
                {
                    isResizing = true;
                    resizeStartPos = currentEvent.mousePosition;
                    resizeStartSize = new Vector2(_windowRect.width, _windowRect.height);
                    currentEvent.Use();
                }
            ...
            case EventType.MouseDrag:
                if (isResizing)
                {
                    Vector2 delta = currentEvent.mousePosition - resizeStartPos;
                    _windowRect.width = Mathf.Max(200, resizeStartSize.x + delta.x);
                    _windowRect.height = Mathf.Max(150, resizeStartSize.y + delta.y);
                    currentEvent.Use();
                }
    ...
    {
        GUI.DragWindow(new Rect(0, 0, 12000, 20));
